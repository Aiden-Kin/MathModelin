...
def buoy_ode(Y, t, params):
    ...
    F_spring = -k * (x_buoy - x_mass)   #弹簧弹力
    F_damping = -C_d *(v_buoy - v_mass)**(3/2) #直线阻尼力
    ...
    # Equations for buoy
    dx_buoy_dt = v_buoy
    dv_buoy_dt = (F_PTO + F_spring + F_damping + F_wave_damping + F_restoring + F_wave) / (M + m_a)
...
k = 1  # 弹簧刚度 (N/m), not given in the problem
C_d = 10000  # 阻尼器阻尼系数 (N·s/m)
C_wave = 656.3616  # 兴波阻尼系数 (N·s/m)
rho = 1000  # Density of water (kg/m^3)
g = 9.81  # Acceleration due to gravity (m/s^2)
A = 1  # Cross-sectional area of the sea level (m^2), assumed
