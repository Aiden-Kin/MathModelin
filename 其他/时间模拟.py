...
def buoy_ode(Y, t, params):
    ...
    F_damping = -C_d * (v_buoy - v_mass)    #直线阻尼力
    F_wave_damping = -C_wave * v_buoy   #兴波阻尼力
    F_restoring = -rho * g * A*np.pi * x_buoy #净水恢复力
    F_wave = F_wave_amp * np.cos(omega * t) #波浪激励力

    # Equations for buoy
    dx_buoy_dt = v_buoy
    dv_buoy_dt = ( F_spring + F_damping + F_wave_damping + F_restoring + F_wave) / (M + m_a)
